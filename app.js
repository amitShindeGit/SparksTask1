...
app.get("/transfer", function(req, res) {
  RegisterUser.find({}, function(err, users) {
      if (err){
          ...
          res.render('transfer', { data: users, cuurentUser : email});
      ...
      if (err){
          ...
          TransferLog.find({}, function(err, logs){
            if(err){
              ...
              res.render('transferlogs', { logData: logs, data: users, cuurentUser : email});
            }
          });
...
app.post("/transfer", function(req, res){
  ...
  RegisterUser.findOne({email : email}, function(err, foundUser){
    ...
    if(foundUser.email === req.body.emailsender){
      ...
      RegisterUser.findOneAndUpdate({email : email}, {"$set" : {"money" : parseInt(userDeductedMoney)}}, {returnNewDocument: true}, (err, doc) => {
        if(err){
          console.log(err);
        }else{
  ...
  RegisterUser.findOneAndUpdate({email : email}, {"$set" : {"money" : parseInt(userDeductedMoney)}}, {returnNewDocument: true}, (err, doc) => {
   ...
   //Transfer to others
  RegisterUser.findOne({email : req.body.emailsender}, function(err, foundUser){
    const sendermoney = parseInt(req.body.sendmoney);
    const userFinalMoney = parseInt(foundUser.money) + sendermoney

    RegisterUser.findOneAndUpdate({email : req.body.emailsender}, {"$set" : {"money" : parseInt(userFinalMoney)}}, {returnNewDocument: true}, (err, doc) => {
      if (err) {
        ...
        RegisterUser.findOne({email : email}, function(err, foundUser){
          ...
          if(err){
            ...
            if(foundUser){
              ...
              RegisterUser.findOneAndUpdate({email : email}, {"$set" : {"money" : parseInt(userFinalMoney)}}, {returnNewDocument: true}, (err, doc) => {
                if (err) {
                  console.log("Something wrong when updating data!");
                }
              ...
              res.render('addmoney', { data: users, addingMoney, name : foundUser.firstName, totalmoney : userFinalMoney, cuurentUser : email});
              }
            }
          })
        }
...
app.get("/", function(req, res) {
    RegisterUser.find({}, function(err, users) {
        if (err){
            ...
            res.render('home', { data: users, cuurentUser : email});
